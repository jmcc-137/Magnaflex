...
const ServicioCard = ({ titulo, descripcion, imagen }) => (
  ...
  >
    <div className="absolute inset-0 bg-black/60 transition duration-300 hover:bg-black/40" />
    <div className="relative z-10 flex flex-col items-center justify-center w-full h-full px-4">
      <h3 className="text-white/95 text-xl md:text-2xl font-bold text-center drop-shadow-lg mb-2">{titulo}</h3>
      <p className="text-white/90 text-base md:text-lg text-center drop-shadow-lg">{descripcion}</p>
    </div>
  </div>
);
...
const ServiciosSection = () => {
  ...
  <section className="w-full py-12 bg-[#23272A] flex flex-col items-center justify-center">
    <h2 className="text-xl sm:text-3xl md:text-4xl font-bold text-center text-white mb-8 max-w-3xl mx-auto leading-tight">
